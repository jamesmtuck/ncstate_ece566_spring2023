...
float test_16(float a, float b, float c, float d, float e, float f,
		     float g, float h, float i);

float test_16_tester(float a, float b, float c, float d, float e, float f,
		     float g, float h, float i)
{
  float arr[9] = {a, b, c, d, e, f, a, b, c};

  float de = determinant((float *)arr, 3, 3);
...
int main()
{
  ...
  for(int i=1; i<100; i++)
    {
      float ret = test_16(i,i+1,i+2,i+3,i+4,i+5,i+6,i+7,i+8);
      float sol = test_16_tester(i,i+1,i+2,i+3,i+4,i+5,i+6,i+7,i+8);
      if ( !compare(ret,sol,0.0001) ) {
	printf("test_16 at %d should be %f, but got %f.\n",i,sol,ret);
	return 1;
      }
    }
